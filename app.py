...
from docx.oxml.ns import qn  # 設定中文字型的重要元件
...
# =========================================================
# 1.1) 先放「現況提醒」（前台白話，明確講遠端控制客戶電腦）
# =========================================================
st.info(
    "現況提醒：目前我的 FB 個人帳號仍然被停用，但我仍需要每天監控你的廣告成果，因此我會教你怎麼每天匯出我需要的數據"
    "（我會幫你設定好，只要每天幫我按下匯出就可以了）。我會依照每天監控狀況判斷是否要跟你約線上遠端控制調整後臺。"
    "為避免耽誤太多時間，遠端前我都會先做好完整調整規劃，實際操控都會非常快。"
)
st.markdown("---")

# =========================================================
# 2) Session state（避免下載後內容消失）
# =========================================================
if "generated" not in st.session_state:
    ...
    st.session_state.last_payment_option = ""
    st.session_state.last_start_date = None
    st.session_state.last_payment_day = None
    st.session_state.last_payment_date = None

# =========================================================
# 3) Word 字型設定函式（強制使用微軟正黑體）
# =========================================================
...
def generate_docx_bytes(party_a, payment_opt, start_dt, pay_day, pay_dt):
    ...
    if payment_opt == "17,000元/月（每月付款）":
        ...
        refund_text = (
            ...
            "即使甲方於合約期間內提前終止或未使用完畢服務內容，亦同；惟因乙方重大違約致服務無法履行者，不在此限。"
        )
    ...
    add_clause("第四條　配合事項與作業方式", [
        ...
        "2. 若因平台政策、帳號狀況或其他不可控因素需採替代作業方式（例如：由甲方每日匯出報表供乙方監控），甲方同意合理配合。",
        "3. 當需調整後台設定時，雙方得約定以線上遠端方式，由乙方於甲方電腦畫面上進行必要之操作與調整。"
    ])
...
st.markdown("""
1. **廣告上架**  
   - 依你的需求，在 Meta（Facebook / Instagram）後台建立並上架廣告活動  
2. **廣告監控 / 維護 / 優化**  
   - 依每日數據監控成效，必要時調整受眾、版位、預算、素材搭配與投放設定  
3. **簡易週報**  
   - 每週提供成果摘要與下週優化方向（讓你知道我做了什麼、下一步要做什麼）
""")

st.subheader("📌 非固定工作（視狀況提供）")
st.markdown("""
1. **廣告素材建議**  
   - 依投放成效、競品動態、市場觀察提出素材方向  
   - 提供文案／圖片內容建議，讓你能快速調整素材  
2. **到達頁面優化建議**  
   - 觀察頁面轉換表現  
   - 當轉換率異常或下降時，提供頁面優化方向
""")

st.warning("📌 稅務提醒：乙方為自然人，無須開立統一發票。甲方可依自身帳務需求選擇是否走勞報或其他合法付款方式。")
st.markdown("---")
...
st.header("🧾 甲方資訊")
party_a_name = st.text_input("甲方名稱（公司或個人）", placeholder="請輸入甲方名稱")

st.header("👤 乙方資訊")
st.text_input("乙方（服務執行者）", value=PROVIDER_NAME, disabled=True)
c1, c2 = st.columns(2)
with c1:
...
方案：45,000元/三個月（季付）
合作啟動日：{start_date.strftime('%Y-%m-%d')}
付款日期：{payment_date.strftime('%Y-%m-%d')} 前
"""
...
"""
        ...
        st.session_state.last_payment_option = payment_option
        st.session_state.last_start_date = start_date
        st.session_state.last_payment_day = payment_day
        st.session_state.last_payment_date = payment_date
...
if st.session_state.generated:
    st.markdown("---")
    st.subheader("📤 給甲方的訊息（請複製後用 LINE 傳給我）")
